  ...
  module FeaturesHelper
    ...
    def draw_aggregate(aggregate, level=1)
      aggregate.keys.sort.collect { |key|
        key.is_a?(SteakFM::Feature) ?
                draw_aggregate_feature(key, aggregate[key]) + scenario_rows(aggregate[key]) :
                (key =~ SteakFM::FeatureElement::Component::Tags::PATTERN[:status] ?
    ...
    # TODO missing information about effort per feature
    def draw_aggregate_feature(feature, scenarios)
      content_tag 'tr', :class => 'feature_header' do
        content_tag('td') <<
                ...
                content_tag('td', scenarios.effort, :style => "text-align:center;")
      end
    end
    ...
    def report_header(collection, name, level)
      ...
      content_tag 'tr', :class => "raport_header level_#{level}#{warning_class}" do
        content_tag('td', name, :colspan => 2) <<
                ...
                content_tag('td', collection.effort, :style => "text-align:center;")
      end
    end
...
#                end <<
        content_tag('td', collection.features.size, :style => "text-align:center;") <<
                ...
                content_tag('td') do
                  content_tag('div', :class => 'progress_bar w170') do
                    content_tag('div', "#{collection.effort_complete_percentage}% from #{collection.effort}",
                                :class => 'percent') <<
                            content_tag('div', :class => "bar_percent",
                                        :style => "width: #{collection.effort_complete_percentage}%") do
                              content_tag('div', '', :class => 'bar')
                            end
                  end
                end
    ...
    def feature_header(feature)
      content_tag 'tr', :class => 'feature_header' do
        content_tag('td') <<
                content_tag('td', link_to(feature.title, edit_documentation_feature_path(feature.id))) <<
                content_tag('td') <<
                content_tag('td', feature.scenarios.size, :style => "text-align:center;") <<
                content_tag('td', feature.effort, :style => "text-align:center;")
      end
    end
    ...
    def scenario_row(scenario)
      content_tag('tr', :class => 'scenario_row') do
        content_tag('td') <<
            ...
            # TODO refactor
                content_tag('td', content_tag('span', scenario.effort, :class => "marker #{scenario.complete? ? 'm_green' : (scenario.status == 'todo' ? 'm_orange' : 'm_red')}"), :style => 'text-align: center;',
                            :class => (scenario.complete? ? 'done' : 'not_done'))
      end
    end
        ...
        content_tag('thead') do
          content_tag('tr') do
            content_tag('th', name.to_s.humanize) <<
                    content_tag('th', 'Effort', :style => 'width: 200px') <<
        ...
        content_tag('td', key) <<
                content_tag('td') do
                  content_tag('div', :class => 'progress_bar') do
                    content_tag('div', "#{element.effort_complete_percentage}% from #{element.effort}", :class => 'percent') <<
                            content_tag('div', :class => 'bar_percent', :style => "width: #{element.effort_complete_percentage}%") do
                              content_tag('div', '', :class => 'bar')
                            end
                  end
                end <<
